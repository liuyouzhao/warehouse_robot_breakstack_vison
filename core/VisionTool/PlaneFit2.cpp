...
PlaneFit2::PlaneFit2(cv::Mat& _color, cv::Mat& _cloud, Size _box, Mat _camera, int _smallRegion, int _gradMax)
    ...
    nextNeighbor(NULL), neighborBucket(NULL), average(NULL), regionIndex(NULL), nodes(NULL), gradMax(_gradMax), boxSize(_box), damnRight(false), color(_color)
{
...
void PlaneFit2::findRightAngle(Mat curve)
{
    ...
    int width = boxSize.width/planeDistance*fxy/2-3;
    int height = boxSize.height/planeDistance*fxy/2-3;
    boxPixelSize.width = width;
    boxPixelSize.height = height;
    int scale = 3;
    ...
    if (rcs.list.size() > 0)
    {
        ...
        case 2:
        {
            ...
            float ratio0 = abs(1.*(2*width - len)) / len / 2;
            float ratio1 = abs(1.*(2*height - len)) / len / 2;
            if (ratio0 < ratio1)
                len = height * 2;
            else
                len = width * 2;
...
void removeRepeatition(Mat edges, vector<Vec3f> lines)
{
    vector<Point> ptList;
...
#if 0
void findBorder(Mat color, Mat mask)
{
    vector<vector<Point>> contours;
    findContours(mask, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
    drawContours(mask, contours, 0, Scalar(255), CV_FILLED);
    Mat mask2, mask3, mask4, mask5;
    Mat ones = Mat::ones(9, 9, CV_8U);
    Mat ones2 = Mat::ones(7, 7, CV_8U);
    Mat ones3 = Mat::ones(5, 5, CV_8U);
    dilate(mask, mask2, ones);
    erode(mask, mask3, ones2);
    mask4 = mask2 - mask3;
    
    Mat edges,edges2;
    Canny(color, edges, 100, 120);
    edges2 = mask4.mul(edges);
    vector<Point> ptList, approCurve;
   
    

    contours.clear();
    Mat plus = edges2 + mask;
    dilate(plus, plus, ones3);
    erode(plus, plus, ones3);
    findContours(plus, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
    approxPolyDP(contours[0], approCurve, 3, true);
    contours.clear();
    contours.push_back(approCurve);
    Mat show = Mat::zeros(mask.size(), mask.type());
    drawContours(show, contours, 0, Scalar(255));
    //show = Mat::zeros(mask.size(), mask.type());
    //drawContours(show, contours, 3, Scalar(255));

    vector<Vec2f> lines0;
    vector<int> score;
    HoughLinesWu(edges2, lines0, score, 1.5,  2.1*CV_PI / 180, 60);
    ...
    int houghthresh0 = 10;
    float houghthresh1 = CV_PI / 40; 
    vector<Vec3f> cluster;
    {
        vector<Vec3f> lineAndScore;
       
        for (int i = 0; i < lines0.size(); i++)
        {
            lineAndScore.push_back(Vec3f(lines0[i][0], lines0[i][1], score[i]));
        }
        sort(lineAndScore.begin(), lineAndScore.end(), [](const Vec3f& a, const Vec3f& b){return a[2] > b[2]; });
        for (int i = 0; i < lineAndScore.size(); i++)
        {
            bool remove = false;
            for (int j = 0; j < cluster.size(); j++)
            {
                if (abs(lineAndScore[i][0] - cluster[j][0]) < houghthresh0 && abs(lineAndScore[i][1] - cluster[j][1]) < houghthresh1)
                {
                    remove = true;
                }
            }
            if (!remove)
            {
                cluster.push_back(lineAndScore[i]);
            }
        }
        Mat color_dst;
        cvtColor(show, color_dst, CV_GRAY2BGR);
        ...
        {
            float rho = cluster[i][0];
            float theta = cluster[i][1];
            double a = cos(theta), b = sin(theta);
            double x0 = a*rho, y0 = b*rho;
            Point pt1(cvRound(x0 + 1000 * (-b)),
                cvRound(y0 + 1000 * (a)));
    ...
    HoughLinesP(edges2, lines, 3, CV_PI / 180, 60, 0,10);
    Mat color_dst;   
    cvtColor(edges2, color_dst, CV_GRAY2BGR);
   for (size_t i = 0; i < lines.size(); i++)
    {
        ...
        line(color_dst, Point(curLine[0], curLine[1]), Point(curLine[2], curLine[3]), Scalar(0, 0, 255), 1, 8);
    }
    printf("");
}
#endif
...
void PlaneFit2::reCalculate(std::vector<cv::Point> vertices)
{
    ...
    Mat mask0 = Mat::zeros(cloud.size(), CV_8U);   
    for (int i = 0; i < planes[planeId].pts.size(); i++)
    {
        int j = planes[planeId].pts[i];
        mask0.data[j] = 1;
    ...
    plane& p0 = planes[planeId];
    plane p;
    for (int i = 0; i < p0.pts.size(); i++)
    {
        int j = p0.pts[i];
        if (mask.data[j] > 0)
        {
            p.pts.push_back(j);
            p.points.push_back(p0.points[i]);
        }
    }    
    cv::Mat mat(p.points.size(), 3, CV_32F, p.points.data());
    cv::Mat planeMat = estPlane(mat); //mat.inv(cv::DECOMP_SVD)*cv::Mat::ones(p.pts.size(), 1, CV_32F);
    ...
    Point3f vs[4];
    for (int i = 0; i < 4;i++)
        solvePtOnPlane(vertices[i], p, vs[i]);
